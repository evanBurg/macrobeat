...
const refreshToken = async () => {
	...
	await AuthToken.findOneAndUpdate({ serviceName: `spotify` }, { authToken }).exec();
};
...
const search = async (searchQuery) => {
	...
	if (body.error && (body.error.status === 401 || body.error.message === `The access token expired`)) {
		await refreshToken();
...
const formatResults = async (res) => {
	...
	for (let i = 0; i < rawSongs.length && i < process.env.QUERY_LIMIT; i++) {
		...
		const image = rawSongs[i].album.images[rawSongs[i].album.images.length - 1].url;
