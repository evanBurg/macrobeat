		...
		request.post(authOptions, (error, response, body) => {
			...
			request.get(options, (error, response, body) => {
				...
				if (body.product !== `premium`) {
					return res.send(`This account does not have access to Spotify Premium.`);
				}
				const doc = { serviceName: `spotify`, authToken: accessToken, refreshToken };
				AuthToken.findOneAndUpdate({ serviceName: `spotify` }, doc, { upsert: true }, (err, doc) => {
					if (err) {
						return res.status(500).send(`Spotify auth token could not be saved.`);
					}
